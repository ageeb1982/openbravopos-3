...
public class TicketLineInfo implements SerializableWrite, SerializableRead, Serializable {
    ...
    private Long m_sTicket;
    private Integer m_iLine;
    private double multiply;
    private double price;
    private TaxInfo tax;
    private Properties attributes;
    private Long productid;
    private Long attsetinstid;

    /** Creates new TicketLineInfo */
    public TicketLineInfo(Long productid, double dMultiply, double dPrice, TaxInfo tax, Properties props) {
        init(productid, null, dMultiply, dPrice, tax, props);
    }

    public TicketLineInfo(Long productid, double dMultiply, double dPrice, TaxInfo tax) {
        init(productid, null, dMultiply, dPrice, tax, new Properties());
    }

    public TicketLineInfo(Long productid, String productname, String producttaxcategory, double dMultiply, double dPrice, TaxInfo tax) {
    ...
    public TicketLineInfo(ProductInfoExt product, double dMultiply, double dPrice, TaxInfo tax, Properties attributes) {
        ...
        if (product == null) {
            ...
            if (product.getAttributeSetID() != null) {
                attributes.setProperty("product.attsetid", product.getAttributeSetID().toString());
            }
            attributes.setProperty("product.taxcategoryid", product.getTaxCategoryID().toString());
            if (product.getCategoryID() != null) {
                attributes.setProperty("product.categoryid", product.getCategoryID().toString());
            }
        }
    ...
    private void init(Long productid, Long attsetinstid, double dMultiply, double dPrice, TaxInfo tax, Properties attributes) {
        ...
        m_sTicket = null;
        m_iLine = Constants.TICKET_LINE_MINUS_ONE;
    }

    void setTicket(Long ticket, int line) {
        m_sTicket = ticket;
        m_iLine = line;
    }

    public void writeValues(DataWrite dp) throws BasicException {
        dp.setLong(1, m_sTicket);
        dp.setInt(2, m_iLine);
        dp.setLong(3, productid);
        dp.setLong(4, attsetinstid);

        dp.setDouble(5, new Double(multiply));
        dp.setDouble(6, new Double(price));

        dp.setLong(7, tax.getId());
        try {
    ...
    public void readValues(DataRead dr) throws BasicException {
        m_sTicket = dr.getLong(1);
        m_iLine = dr.getInt(2);
        productid = dr.getLong(3);
        attsetinstid = dr.getLong(4);

        multiply = dr.getDouble(5);
        price = dr.getDouble(6);

        tax = new TaxInfo(dr.getLong(7), dr.getString(8), dr.getLong(9), dr.getTimestamp(10), dr.getLong(11), dr.getLong(12), dr.getDouble(13), dr.getBoolean(14), dr.getInt(15));
    ...
    public Long getProductAttSetId() {
        String attsetid = attributes.getProperty("product.attsetid");
        if (attsetid == null) {
          return null;
        } else {
          return new Long(attsetid);
        }
    }
    ...
    public Long getProductAttSetInstId() {
        return attsetinstid;
    }

    public void setProductAttSetInstId(Long value) {
        attsetinstid = value;
    }
    ...
    public Long getProductTaxCategoryID() {
      String taxcategoryid = attributes.getProperty("product.taxcategoryid");
      if (taxcategoryid == null) {
        return null;
      } else {
        return Long.valueOf(taxcategoryid);
      }
    }
