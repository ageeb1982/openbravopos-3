...
public class TicketInfo implements SerializableRead, Externalizable {
  ...
  public TicketInfo() {
    ...
    m_iTicketId = Constants.TICKET_ZERO; // incrementamos
    m_dDate = new Date();
  ...
  public void readValues(DataRead dr) throws BasicException {
    ...
    m_iTicketId = dr.getLong(3);
    m_dDate = dr.getTimestamp(4);
    m_sActiveCash = dr.getLong(5);
    try {
    ...
    m_User = new UserInfo(dr.getLong(7), dr.getString(8));
    m_Customer = new CustomerInfoExt(dr.getLong(9));
  ...
  public String getTransactionID() {
    String transaction = (getPayments().size() > 0) ? 
  ...
  public void setActiveCash(Long value) {
    m_sActiveCash = value;
  }

  public Long getActiveCash() {
  ...
  public double getTotalPaid() {
    ...
    double sum = 0.0;
    for (PaymentInfo p : payments) {
      if (!p.getPaymentMethodId().equals(PaymentMethod.DEBT_PAID.getKey())) {
        sum += p.getTotal();
      }
    }
    return sum;
  ...
  public TicketTaxInfo[] getTaxLines() {
    ...
    Map<Long, TicketTaxInfo> m = new HashMap<Long, TicketTaxInfo>();
  ...
  public String printId() {
    if (m_iTicketId > 0) {
      // valid ticket id
      return Formats.LONG.formatValue(m_iTicketId);
    } else {
      return "";
    }
  }
